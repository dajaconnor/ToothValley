...
public class EnvironmentService {
	...
	public void shiftTectonics() {
		...
		if (map.getPlates().size() > 0) {
			...
			int i = 0;
			int leftoverElev = 0;
			int additionalActivity = UserActions.getInstance().getAndResetTectonicActivity();
			int activity = Environment.TECTONIC_ACTIVITY + Environment.TECTONIC_ACTIVITY * additionalActivity;
